    ...
    void agressive_early_deflation(bool want_t, bool want_z, idx_t ilo, idx_t ihi, idx_t nw, matrix_t &A, vector_t &s, matrix_t &Z, idx_t &ns, idx_t &nd, francis_opts_t<size_type<matrix_t>, type_t<matrix_t>> &opts)
    {
        ...
        if (ihi < istop_m)
        {
            ...
            while (i < istop_m)
            {
                ...
                matrix_multiply(Op::ConjTrans, Op::NoTrans, one, V, A_slice, WH_slice);
        ...
        if (istart_m < kwtop)
        {
            ...
            while (i < kwtop)
            {
                ...
                matrix_multiply(Op::NoTrans, Op::NoTrans, one, A_slice, V, WV_slice);
        ...
        if (want_z)
        {
            ...
            while (i < n)
            {
                ...
                matrix_multiply(Op::NoTrans, Op::NoTrans, one, Z_slice, V, WV_slice);
