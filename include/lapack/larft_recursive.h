...
// assigning the blocks 
    ...
    auto T11 = tlapack::slice( TTT, range(n0,k), range(n0,k) );
    auto T01 = tlapack::slice( TTT, range(0,n0), range(n0,k) );


    // slicing tau (one dimention)
    auto tau0 = tlapack::slice( tau, range(0,n0) );
    auto tau1 = tlapack::slice( tau, range(n0,k) );
    ...
    // Step 1 ---- (V10)^H -> T01
    auto A10 = tlapack::slice(A, range(n0, k), range(0, n0));
    ...
    // step 2--- T01 * V11 -> T01
    auto A11 = tlapack::slice(A, range(n0, k), range(n0, k));
