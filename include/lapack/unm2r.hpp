    ...
    work_t& work )
{
    ...
    tlapack_error_if( trans == Op::Trans && is_complex<matrixA_t>::value, -2 );
    tlapack_error_if( access_denied( lowerTriangle, read_policy(A)  ), -3 );
    tlapack_error_if( access_denied( band_t(0,0),   write_policy(A) ), -3 );
    tlapack_error_if( access_denied( dense, write_policy(C) ), -5 );
