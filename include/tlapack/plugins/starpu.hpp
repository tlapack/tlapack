...
/**
 * @brief Data operation with assignment using a StarPU variable buffer and a
 * value
 *
 * This function is used to perform data operations on Matrix<T>::data.
 * The interface is suitable for tasks that are submitted to StarPU.
 ...
 * a simple interface to perform data creation, partitioning, and entry access.
 *
 * @tparam T Data type
 */
template <class T>
class Matrix {
   public:
    ...
    /**
     * @brief Create a submatrix when the matrix is not partitioned
     *
     */
    // constexpr Matrix get_submatrix(idx_t i, idx_t j, idx_t m, idx_t n) noexcept
    // {
    //     assert(!is_partitioned && "Matrix is already partitioned");
    //     assert(m >= 0 && n >= 0 && "Number of tiles must be positive or 0");
    //     assert((i >= 0 && i + m <= nrows()) && "Submatrix out of bounds");
    //     assert((j >= 0 && j + n <= ncols()) && "Submatrix out of bounds");
    ...
    //     /// TODO: This is not the correct function to use. It only works if m is
    //     /// a multiple of nx and n is a multiple of ny. We may need to use
    //     /// another filter.
    ...
    //             ? starpu_data_get_sub_data(handle, 2, ix + i / mb, iy + j / nb)
    //             : handle;
    ...
    // constexpr Matrix<const T> get_constMatrix(idx_t ix, idx_t iy, idx_t nx,
    // idx_t ny) const noexcept
    // {
    //     return Matrix<const T>(handle, is_partitioned, ix, iy, nx, ny);
    // }

    // template <class U>
    // constexpr Matrix<const T> create_constMatrix(
    //     const Matrix<U>& A, idx_t ix, idx_t iy, idx_t nx, idx_t ny) noexcept
    //     : handle(A.handle), is_owner(false), ix(ix), iy(iy), nx(nx), ny(ny)
    // {
    //     assert(A.is_partitioned && "Matrix is not partitioned");
    //     assert(nx >= 0 && ny >= 0 && "Number of tiles must be positive or
    //     0"); assert((ix >= 0 && ix + nx <= A.nx) && "Submatrix out of
    //     bounds"); assert((iy >= 0 && iy + ny <= A.ny) && "Submatrix out of
    //     bounds");
    // }
     ...
     *
     * This function creates a grid that partitions the matrix into nx*ny tiles.
     * If the matrix is m-by-n, then every tile (i,j) from 0 <= i < m-1 and 0 <=
     * j < n-1 is a matrix (m/nx)-by-(n/ny). The tiles where i = nx-1 or j =
     * ny-1 are special, as they may have a smaller sizes.
     *
    ...
    void create_grid(idx_t nx, idx_t ny) noexcept
    {
        ...
        assert(nx >= 0 && ny >= 0 && "Number of tiles must be positive or 0");
    ...
    {
        return starpu_matrix_get_nx((starpu_data_get_nb_children(handle) > 0)
    ...
    constexpr idx_t nblockcols() const noexcept
    {
        if (starpu_data_get_nb_children(handle) > 0) {
            const starpu_data_handle_t x0 = starpu_data_get_child(handle, 0);
     ...
     *
     */
    constexpr Matrix get_tiles(idx_t ix, idx_t iy, idx_t nx, idx_t ny) noexcept
    {
        assert(is_partitioned && "Matrix is not partitioned");
        assert(nx >= 0 && ny >= 0 && "Number of tiles must be positive or 0");
        assert((ix >= 0 && ix + nx <= this->nx) && "Submatrix out of bounds");
        assert((iy >= 0 && iy + ny <= this->ny) && "Submatrix out of bounds");

        return Matrix(handle, is_partitioned, this->ix + ix, this->iy + iy, nx,
                      ny);
    }

    /**
     * @brief Create a const submatrix when the matrix is partitioned into tiles
     *
     * @param[in] ix Index of the first tile in x
     * @param[in] iy Index of the first tile in y
     * @param[in] nx Number of tiles in x
    ...
    /// @brief Matrix destructor unpartitions and unregisters the data handle
...
struct Matrix<T>::data {
    ...
    constexpr data& operate_and_assign(const data& x) noexcept
    {
        ...
        const int ret =
            starpu_task_insert(&cl, STARPU_W, this->handle, STARPU_R, x.handle,
    ...
    constexpr friend std::ostream& operator<<(std::ostream& out, const data& x)
    {
        return out << T(x);
    }
};
                     ...
                     SliceSpecCol&& cols)
{
    ...
    if (A.is_partitioned()) {
        ...
        return A.get_const_tiles(
            row0 / mb, col0 / nb,
            (nrows == 0) ? 0 : std::min<uint32_t>(nrows / mb, 1),
            (ncols == 0) ? 0 : std::min<uint32_t>(ncols / nb, 1));
    }
    else {
        /// TODO: Do something here!!!!
        return A.get_const_tiles(row0, col0, nrows, ncols);
    }
}
template <
    class T,
