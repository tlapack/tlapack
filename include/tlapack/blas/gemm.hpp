...
#if defined(TLAPACK_USE_MKL) && defined(MKL_BF16) && defined(EIGEN_BFLOAT16_H)

template <class idx_t, Layout L>
void gemm(Op transA,
          Op transB,
          float alpha,
          const tlapack::LegacyMatrix<Eigen::bfloat16, idx_t, L>& A,
          ...
          tlapack::LegacyMatrix<float, idx_t, L>& C)
{
    ...
    cblas_gemm_bf16bf16f32(layout, CblasNoTrans, CblasNoTrans, m, n, k, alpha,
                           A_.ptr, A_.ldim, B_.ptr, B_.ldim, beta, C_.ptr,
                           C_.ldim);
}
