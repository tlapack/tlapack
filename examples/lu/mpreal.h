...
#else
    #define IsInf(x) \
        std::isinf(x)  // GNU C/C++ (and/or other compilers), just hope for C99
                       // conformance
#endif
...
class mpreal {
   ...
   public:
    ...
    friend const mpreal root(const mpreal& v,
                             unsigned long int k,
                             mp_rnd_t rnd_mode);
    friend const mpreal pow(const mpreal& a,
                            const mpreal& b,
                            mp_rnd_t rnd_mode);
    ...
    friend const mpreal dim(const mpreal& a,
                            const mpreal& b,
                            mp_rnd_t rnd_mode);
    friend inline const mpreal mul_2ui(const mpreal& v,
                                       unsigned long int k,
                                       mp_rnd_t rnd_mode);
    friend inline const mpreal mul_2si(const mpreal& v,
                                       long int k,
                                       mp_rnd_t rnd_mode);
    friend inline const mpreal div_2ui(const mpreal& v,
                                       unsigned long int k,
                                       mp_rnd_t rnd_mode);
    friend inline const mpreal div_2si(const mpreal& v,
                                       long int k,
                                       mp_rnd_t rnd_mode);
    ...
    friend const mpreal atan2(const mpreal& y,
                              const mpreal& x,
                              mp_rnd_t rnd_mode);
    ...
    friend const mpreal hypot(const mpreal& x,
                              const mpreal& y,
                              mp_rnd_t rnd_mode);
    ...
    friend const mpreal fma(const mpreal& v1,
                            const mpreal& v2,
                            const mpreal& v3,
                            mp_rnd_t rnd_mode);
    friend const mpreal fms(const mpreal& v1,
                            const mpreal& v2,
                            const mpreal& v3,
                            mp_rnd_t rnd_mode);
...
// MPFR 2.4.0 Specifics
#if (MPFR_VERSION >= MPFR_VERSION_NUM(2, 4, 0))
    friend int sinh_cosh(mpreal& s,
                         mpreal& c,
                         const mpreal& v,
                         mp_rnd_t rnd_mode);
    friend const mpreal li2(const mpreal& v, mp_rnd_t rnd_mode);
    friend const mpreal fmod(const mpreal& x,
                             const mpreal& y,
                             mp_rnd_t rnd_mode);
    ...
    friend const mpreal remainder(const mpreal& x,
                                  const mpreal& y,
                                  mp_rnd_t rnd_mode);
    friend const mpreal remquo(const mpreal& x,
                               const mpreal& y,
                               int* q,
                               mp_rnd_t rnd_mode);
...
// MPFR < 2.4.2 Specifics
    ...
    friend const mpreal fmax(const mpreal& x,
                             const mpreal& y,
                             mp_rnd_t rnd_mode);
    friend const mpreal fmin(const mpreal& x,
                             const mpreal& y,
                             mp_rnd_t rnd_mode);
...
{
    if (this != &other) {
        mpfr_swap(mpfr_ptr(), other.mpfr_ptr());  // destructor for "other" will
                                                  // be called just afterwards
        MPREAL_MSVC_DEBUGVIEW_CODE;
    }
    return *this;
}
...
inline mpreal::mpreal(const unsigned long long int u,
                      mp_prec_t prec,
                      mp_rnd_t mode)
{
    mpfr_init2(mpfr_ptr(), prec);
...
inline mpreal::mpreal(const std::string& s,
                      mp_prec_t prec,
                      int base,
                      mp_rnd_t mode)
{
...
namespace internal {
    ...
    // Use SFINAE to restrict arithmetic operations instantiation only for
    // numeric types This is needed for smooth integration with libraries based
    // on expression templates, like Eigen.
...
const mpreal pow(const unsigned int a,
                 const mpreal& b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long int a,
                 const mpreal& b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const int a,
                 const mpreal& b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const long double a,
                 const mpreal& b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
const mpreal pow(const double a,
                 const mpreal& b,
                 mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
inline const mpreal mul_2ui(const mpreal& v,
                            unsigned long int k,
                            mp_rnd_t rnd_mode = mpreal::get_default_rnd());
inline const mpreal mul_2si(const mpreal& v,
                            long int k,
                            mp_rnd_t rnd_mode = mpreal::get_default_rnd());
inline const mpreal div_2ui(const mpreal& v,
                            unsigned long int k,
                            mp_rnd_t rnd_mode = mpreal::get_default_rnd());
inline const mpreal div_2si(const mpreal& v,
                            long int k,
                            mp_rnd_t rnd_mode = mpreal::get_default_rnd());
...
inline mpreal& mpreal::operator=(const char* s)
{
    ...
    //        mode) mpreal(const std::string& s,mp_prec_t prec, int base,
    //        mp_rnd_t mode)
    //
...
{
    mpreal c(0, (std::max)(mpfr_get_prec(a.mpfr_ptr()),
                           mpfr_get_prec(b.mpfr_ptr())));
...
{
    // TODO: Add extended format specification (f, e, rounding mode) as it done
    // in output operator
    (void)b;
    (void)mode;
...
inline std::ostream& mpreal::output(std::ostream& os) const
{
    ...
    if (!(mpfr_asprintf(&s, format.str().c_str(),
                        mpfr::mpreal::get_default_rnd(), mpfr_srcptr()) < 0)) {
        os << std::string(s);
        mpfr_free_str(s);
    }
    return os;
...
inline const mpreal frexp(const mpreal& x,
                          mp_exp_t* exp,
                          mp_rnd_t mode = mpreal::get_default_rnd())
{
    mpreal y(x);
...
//////////////////////////////////////////////////////////////////////////
// C++11 sign functions.
inline mpreal copysign(const mpreal& x,
                       const mpreal& y,
                       mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline mpreal& setsignbit(mpreal& x,
                          bool minus,
                          mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal root(const mpreal& x,
                         unsigned long int k,
                         mp_rnd_t r = mpreal::get_default_rnd())
{
    mpreal y(0, mpfr_get_prec(x.mpfr_srcptr()));
...
inline const mpreal dim(const mpreal& a,
                        const mpreal& b,
                        mp_rnd_t r = mpreal::get_default_rnd())
{
...
#if (MPFR_VERSION >= MPFR_VERSION_NUM(4, 0, 0))
inline const mpreal gammainc(const mpreal& a,
                             const mpreal& x,
                             mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    /*
...
inline const mpreal beta(const mpreal& z,
                         const mpreal& w,
                         mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    /*
...
/* f(x,u) = f(2*pi*x/u) */
inline const mpreal cosu(const mpreal& x,
                         unsigned long u,
                         mp_rnd_t r = mpreal::get_default_rnd())
{
    MPREAL_BINARY_MATH_FUNCTION_UI_BODY(cosu, u);
}
inline const mpreal sinu(const mpreal& x,
                         unsigned long u,
                         mp_rnd_t r = mpreal::get_default_rnd())
{
    MPREAL_BINARY_MATH_FUNCTION_UI_BODY(sinu, u);
}
inline const mpreal tanu(const mpreal& x,
                         unsigned long u,
                         mp_rnd_t r = mpreal::get_default_rnd())
{
    MPREAL_BINARY_MATH_FUNCTION_UI_BODY(tanu, u);
}
inline const mpreal acosu(const mpreal& x,
                          unsigned long u,
                          mp_rnd_t r = mpreal::get_default_rnd())
{
    MPREAL_BINARY_MATH_FUNCTION_UI_BODY(acosu, u);
}
inline const mpreal asinu(const mpreal& x,
                          unsigned long u,
                          mp_rnd_t r = mpreal::get_default_rnd())
{
    MPREAL_BINARY_MATH_FUNCTION_UI_BODY(asinu, u);
}
inline const mpreal atanu(const mpreal& x,
                          unsigned long u,
                          mp_rnd_t r = mpreal::get_default_rnd())
{
...
inline const mpreal atan2u(const mpreal& y,
                           const mpreal& x,
                           unsigned long u,
                           mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal atan2pi(const mpreal& y,
                            const mpreal& x,
                            mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal powr(const mpreal& x,
                         const mpreal& y,
                         mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal compound(const mpreal& x,
                             long n,
                             mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    /* compound(x,n) = (1+x)^n */
...
inline const mpreal fmod(const mpreal& x,
                         unsigned long u,
                         mp_rnd_t r = mpreal::get_default_rnd())
{
...
inline const mpreal atan2(const mpreal& y,
                          const mpreal& x,
                          mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal a(0, (std::max)(y.getPrecision(), x.getPrecision()));
...
inline const mpreal hypot(const mpreal& x,
                          const mpreal& y,
                          mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal a(0, (std::max)(y.getPrecision(), x.getPrecision()));
...
inline const mpreal hypot(const mpreal& a,
                          const mpreal& b,
                          const mpreal& c,
                          const mpreal& d)
{
...
inline const mpreal remainder(const mpreal& x,
                              const mpreal& y,
                              mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal a(0, (std::max)(y.getPrecision(), x.getPrecision()));
...
inline const mpreal remquo(const mpreal& x,
                           const mpreal& y,
                           int* q,
                           mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal lgamma(const mpreal& v,
                           int* signp = 0,
                           mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal x(v);
...
inline const mpreal besseljn(long n,
                             const mpreal& x,
                             mp_rnd_t r = mpreal::get_default_rnd())
{
    mpreal y(0, x.getPrecision());
...
inline const mpreal besselyn(long n,
                             const mpreal& x,
                             mp_rnd_t r = mpreal::get_default_rnd())
{
    mpreal y(0, x.getPrecision());
...
inline const mpreal fma(const mpreal& v1,
                        const mpreal& v2,
                        const mpreal& v3,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal fms(const mpreal& v1,
                        const mpreal& v2,
                        const mpreal& v3,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal agm(const mpreal& v1,
                        const mpreal& v2,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal a;
...
inline const mpreal sum(const mpreal tab[],
                        const unsigned long int n,
                        int& status,
                        mp_rnd_t mode = mpreal::get_default_rnd())
{
...
inline const mpreal rem(const mpreal& x,
                        const mpreal& y,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
...
inline const mpreal mod(const mpreal& x,
                        const mpreal& y,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    (void)rnd_mode;
...
inline const mpreal fmax(const mpreal& x,
                         const mpreal& y,
                         mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal a;
...
inline const mpreal fmin(const mpreal& x,
                         const mpreal& y,
                         mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal a;
...
inline const mpreal pow(const mpreal& a,
                        const mpreal& b,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal x(a);
...
inline const mpreal pow(const unsigned long int a,
                        const mpreal& b,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd())
{
    mpreal x(a);
...
inline const mpreal pow(const unsigned int a,
                        const mpreal& b,
                        mp_rnd_t rnd_mode)
{
...
// pow long int
inline const mpreal pow(const long int a,
                        const unsigned long int b,
                        mp_rnd_t rnd_mode)
{
    if (a > 0)
...
// pow int
inline const mpreal pow(const int a,
                        const unsigned long int b,
                        mp_rnd_t rnd_mode)
{
    if (a > 0)
...
// pow long double
inline const mpreal pow(const long double a,
                        const long double b,
                        mp_rnd_t rnd_mode)
{
...
inline const mpreal pow(const double a,
                        const unsigned long int b,
                        mp_rnd_t rnd_mode)
{
    return pow(mpreal(a), b, rnd_mode);  // mpfr_pow_ui
...
class numeric_limits<mpfr::mpreal> {
   public:
    ...
    // Following members should be constant according to standard, but they can
    // be variable in MPFR So we define them as functions here.
    //
    ...
    // If possible, please use functions digits() and round_style() defined
    // above.
    //
