...
struct scaled_accessor {
    ...
    constexpr scaled_accessor() noexcept : scale_(1){};

    constexpr scaled_accessor(const scalar_t& scale) noexcept : scale_(scale){};

    template <
        class OtherElementType,
        class otherScalar_t,
        enable_if_t<
            (is_convertible_v<typename scaled_accessor<OtherElementType,
                                                       otherScalar_t>::pointer,
                              pointer> &&
             is_convertible_v<
                 typename scaled_accessor<OtherElementType,
                                          otherScalar_t>::scalar_ptr,
                 scalar_ptr>),
            bool> = true>
    constexpr scaled_accessor(
        scaled_accessor<OtherElementType, otherScalar_t>) noexcept
    {}

    constexpr pointer offset(pointer p, std::size_t i) const noexcept
    {
        return p + i;
    }
    ...
    constexpr scalar_type scale() const noexcept { return scale_; }
 ...
 *
 * It is a lazy evaluation strategy. The actual scale occurs element-wisely at
 * the evaluation on each
 *
 * @tparam scalar_t Type of the scalar multiplying the data
...
template <class scalar_t,
          ...
          class LP,
          class AP,
          enable_if_t<is_same_v<AP, default_accessor<T> >, bool> = true>
...
template <
    ...
    class AP,
    class otherScalar_t,
    enable_if_t<is_same_v<AP, scaled_accessor<T, otherScalar_t> >, bool> = true>
constexpr auto scale(
    const scalar_t& alpha,
    const mdspan<T, std::experimental::extents<Exts...>, LP, AP>& A)
{
