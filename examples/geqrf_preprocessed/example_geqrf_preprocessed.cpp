...
// 3) computing an QR factorization of the transformed matrix (HA)ᴴ
// 4) and using the QR factors to obtain the solution.
//
...
void run(size_t m, size_t n)
{
    ...
    auto startQR = std::chrono::high_resolution_clock::now();
    {
        ...
        // 3) Compute the QR factorization of (HA)ᴴ
        //    2*m*n² - 2/3*n³ FLOPS
        ...
        //    n FLOPS
        T scale = b[m - 1] / conj(HA_T(n - 1, m - 1));
