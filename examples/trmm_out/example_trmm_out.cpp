...
auto trmm_out_cheat(Op transA,
                    ...
                    matrixC_t& C)
{
    using idx_t = tlapack::size_type<matrixC_t>;

    idx_t n = nrows(B);

    std::vector<T> tri_copy_(n * n);
    tlapack::LegacyMatrix<T> tri_copy(n, n, &tri_copy_[0], n);
    ...
    std::cout << "multiply A rows check " << nrows(A) << " rows B " << nrows(B)
              << std::endl;

    gemm(transA, transB, alpha, A, tri_copy, beta, C);
}
...
void run(size_t n, size_t m)
{
    ...
    real_t normAbefore = lantr(Norm::Fro, Uplo::Lower, Diag::NonUnit, A);
    ...
    trmm_out(Side::Right, Uplo::Lower, Op::NoTrans,
             Diag::NonUnit, Op::ConjTrans, alpha, A, B, beta, C);
    ...
    gemm(Op::ConjTrans, Op::NoTrans, alpha, B, A, beta, C_copy);
